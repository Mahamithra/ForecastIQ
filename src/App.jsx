...
/* ============================== AUTH / "DATABASE" LAYER ==============================
   Demo-only auth persisted in the browser's localStorage (works on any static host,
   including Vercel — no backend required). Accounts are local to each visitor's
   browser rather than shared across users. Passwords are never stored in plain text —
   only a SHA-256 hash. Swap dbGetUser/dbSaveUser for real API calls to use a proper
   backend/database instead. ============================== */
const AUTH_KEY_PREFIX = "forecastiq_user:";
const SESSION_KEY = "forecastiq_session_email";
...
async function dbGetUser(email) {
  try {
    const raw = localStorage.getItem(AUTH_KEY_PREFIX + email.toLowerCase());
...
async function dbSaveUser(user) {
  try {
    localStorage.setItem(AUTH_KEY_PREFIX + user.email.toLowerCase(), JSON.stringify(user));
  } catch (e) {}
}
